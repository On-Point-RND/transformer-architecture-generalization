...
All variants use the normal Task/DatasetItem contract.  Distribution shifts are
constructor parameters, so the same trained checkpoint can be evaluated ID/OOD
by overriding ``length_range``, ``target_position_range``, ``distance_range``,
``filler_range`` or ``correlation``.
"""
...
class PositionalLabTask(Task):
    ...
    def __init__(self, task="content_addressed_retrieval", length_range=(16, 32),
        ...
        if task == "local_latest_update" and self.length_range[0] < 8:
            raise ValueError("local_latest_update needs length_range starting at 8 or more")
        if task == "selective_count" and self.CLASS_LOW + self.relevant_count_range[1] >= self.vocab_size:
    ...
    def _draw_length_and_distance(self):
        """Draw (length, distance) until the marked position fits the sequence.

        length and distance are independent knobs, so an OOD ``distance_range``
        can exceed ``length_range``; redraw instead of failing mid-run. The
        invariant ``distance + 2 <= length`` is what both callers below need.
        """
        ...
        raise ValueError(
            f"distance_range={self.distance_range} never fits length_range="
            f"{self.length_range} (need distance + 2 <= length); widen the "
            f"lengths or shorten the distances"
        )
    ...
    def _relative_copy(self):
        length, distance = self._draw_length_and_distance()
        prompt = self._fill(length)
    ...
    TASKS = {
        ...
        "dense_absolute_position": _absolute,
        "relative_offset_copy": _relative_copy,
        "unseen_long_relative_offset": _relative_copy,
        "bucketed_relative_distance": lambda self: self._relative_class(exact=False),
        "exact_distance_beyond_bucket": lambda self: self._relative_class(exact=True),
        "content_addressed_retrieval": _content_retrieval,
        "distant_retrieval": lambda self: self._content_retrieval(distant=True),
        "context_adaptive_retrieval": lambda self: self._content_retrieval(correlation=1),
        "context_correlation_flip": lambda self: self._content_retrieval(correlation=self.correlation),
        "local_latest_update": _latest_update,
        "selective_count": _selective_count,
        "periodic_distance": _periodic_distance,
        "nonperiodic_absolute_threshold": _threshold_position,
    }
