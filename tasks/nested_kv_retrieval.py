...
Token layout — identical to tasks/kv_retrieval.py
------------------------------------------------
This class subclasses ``KVRetrievalTask`` on purpose: it inherits the exact
same special-token ids (PAD/QUERY_MARKER/SOD/EOD/BOE/EOE), the same key/value
id ranges and therefore the same ``vocab_size`` for a given (k_card, v_card).
Nesting reuses SOD/EOD as the brackets around the inner dict rather than
introducing new token ids. A checkpoint trained on the flat task can
consequently be fed these prompts unchanged — no embedding-table mismatch, no
re-tokenisation.

Using it as an OOD test for flat-dict models
--------------------------------------------
The constructor takes the flat task's parameters (`k_card`, `v_card`,
`n_pairs`, `duplicate_keys`, `seed`) with the same names, so a checkpoint's
recorded `task.params` can be reused and overridden for OOD evaluation.
...
the biggest ``max_nested_len`` that always fits. Pass that value and the task
...
class NestedKVRetrievalTask(KVRetrievalTask):
    def __init__(
        self,
        k_card: int,
        v_card: int,
    ...
    def _draw(self, spec) -> int:
        """Resolve an int-or-[lo, hi) spec (the flat task's n_pairs convention)."""
    ...
    def largest_nested_len(cls, n_pairs, n_nested, v_card, block_size,
        ...
        ``max_nested_len`` and the task cannot raise."""
    ...
    def _draw_layout(self):
        """(n_pairs, nested_slots, nested_lens) for one prompt.

        ``nested_slots`` stays a set and ``nested_lens`` a comprehension over it:
        the iteration order of that set decides which slot gets which length, so
        it is part of the random stream and must not be reordered.
        """
        ...
        return n_pairs, nested_slots, nested_lens

    def _check_value_budget(self, n_values, n_pairs, n_nested):
        if n_values <= self.v_card:
            return
        largest = self.largest_nested_len(n_pairs, n_nested, self.v_card, 10**9,
                                          self.query_mode)
        raise ValueError(
            ...
            f"Cap the nested dicts instead of hoping: max_nested_len={largest}"
            f" is the largest that always fits this v_card (0 = lower n_pairs/n_nested"
            f" or raise v_card). See worst_case()."
        )

    def _check_key_budget(self, max_dict_len, key_pool, reserved):
        if max_dict_len <= len(key_pool):
            return
        raise ValueError(
            f"a dict in this prompt holds {max_dict_len} pairs but only "
            f"{len(key_pool)} distinct keys are available (k_card={self.k_card}"
            f"{', minus the reserved query key' if reserved else ''}) — "
            f"raise k_card, lower n_pairs, or set duplicate_keys=True"
        )

    def _emit_nested_dict(self, tokens, top_key, m, key_pool, values, paths):
        """Append ``SOD (BOE key value EOE)*m EOD`` and record two-key paths.

        ``values`` is an iterator over the prompt's globally unique value tokens.
        """
        inner_keys = self.rng.choice(key_pool, size=m, replace=self.duplicate_keys)
        tokens.append(self.SOD_ID)
        for j in range(m):
            ik, iv = int(inner_keys[j]), int(next(values))
    ...
    def _sample_one(self) -> DatasetItem:
        ...
        if not self.duplicate_keys:
            self._check_key_budget(max_dict_len, key_pool, q_key is not None)

        tokens: List[int] = [self.SOD_ID]
        paths: Dict[int, List[int]] = {}  # value token -> key path (1 or 2 keys)
        flat_values: Dict[int, int] = {}  # top-level slot -> its plain value
        ...
        for i in range(n_pairs):
            ...
            if i in nested_slots:
                self._emit_nested_dict(tokens, k, nested_lens[i], key_pool, values, paths)
            else:
                v = int(next(values))
                tokens.append(v)
                paths[v] = [k]
        ...
        query, answer = self._query(paths, flat_values, q_slot, q_key)
        tokens.append(query)
    ...
    def _query(self, paths, flat_values, q_slot, q_key):
        if self.query_mode == "key2value":
            return q_key, [flat_values[q_slot]]
        queried = list(paths)[int(self.rng.integers(len(paths)))]
        return queried, paths[queried]
