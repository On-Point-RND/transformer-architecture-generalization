...
class DyckTask(Task):
    ...
    def _bracket(self, stack: List[int], do_open: bool) -> int:
        if do_open:
            j = int(self.rng.integers(self.n_types))
            stack.append(j)
            return self.open_token_ids[j]
        return self.close_token_ids[stack.pop()]

    def _draw_prefix(self, length: int):
        """One random legal prefix; returns (tokens, still-open stack).

        The coin is only flipped when both moves are legal — otherwise the move
        is forced, and flipping would change the random stream.
        """
        prefix = np.empty(length, dtype=np.int64)
        stack: List[int] = []
        for t in range(length):
            can_open = len(stack) < self.max_depth
            forced = not (can_open and stack)
            do_open = can_open if forced else self.rng.random() < self.p_open
            prefix[t] = self._bracket(stack, do_open)
        return prefix, stack
    ...
    def _sample_one(self) -> DatasetItem:
        ...
        for _ in range(MAX_ATTEMPTS):
            prefix, stack = self._draw_prefix(length)
            if stack:
                return self._item(prefix, stack)
        raise RuntimeError(
            f"no prefix of length {length} left a bracket open in {MAX_ATTEMPTS} "
            f"draws (p_open={self.p_open}, max_depth={self.max_depth}); raise "
            f"p_open or use an odd prefix_len"
        )

    def _item(self, prefix, stack) -> DatasetItem:
